...
(async () => {
  ...
  await context.route(
    "https://wbbatgbvizhghtkvuguf.supabase.co/functions/v1/create-razorpay-order",
    async (route) => {
  ...
  await context.route(
    "https://wbbatgbvizhghtkvuguf.supabase.co/functions/v1/shiprocket-api",
    async (route) => {
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          shiprocket_order_id: "sr_mock_123",
          shiprocket_shipment_id: "sr_shipment_123",
        }),
      });
    },
  );
  ...
  try {
    ...
    const checkoutBtn = page
      .locator("text=/Sign in to check out/i, text=/Checkout/i, text=/Proceed/i")
      .first();
    ...
    if ((await addressInput.count()) > 0) {
      ...
      try {
        await page.fill("input[name='full_name'], input[name='name']", "Test Admin User");
      } catch (e) {}
      try {
        await page.fill("input[name='phone'], input[name='mobile']", "9999999999");
      } catch (e) {}
      try {
        await page.fill("input[name='address'], input[name='street']", "123 Test St");
      } catch (e) {}
      try {
      ...
      try {
        await page.fill("input[name='pincode'], input[name='zip']", "123456");
      } catch (e) {}

      // Click Prepaid option (usually Razorpay)
      const prepaidOption = page
        .locator("text=/Prepaid/i, text=/Razorpay/i, text=/Pay Online/i")
        .first();
      ...
      const placeOrderBtn = page
        .locator(
          "button:has-text('Place Order'), button:has-text('Confirm Order'), button:has-text('Pay Now')",
        )
        .first();
