...
export function DashboardTab({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const stats = useMemo(() => {
    ...
    const offlineOrders = [
      ...orders.filter((o) => o.notes?.includes("POS Order")),
      ...posSales,
    ];
    ...
    return { revenue: rawRevenue, orders: totalOrdersCount, visitors: visitorsCount, lowStock: lowStockCount, avgOrder, stockValue, chartDays, onlineSales, cashSales };
  ...
  const recentActivity = useMemo(() => {
    ...
    visitors.slice(0, 3).forEach((v) => {
      ...
      activities.push({ title: loc ? `New visitor from ${loc}` : "New visitor session", time: format(new Date(v.created_at), "MMM dd, hh:mm a"), icon: Users, color: "text-cyan-500 bg-cyan-50" });
    });
    ...
    <div className="space-y-6 text-[#1e293b]">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div className="relative overflow-hidden rounded-2xl bg-[#16a34a] p-5 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex justify-between items-start">
            ...
            <div className="opacity-20"><span className="text-4xl font-serif">₹</span></div>
          </div>
          <div className="flex items-center justify-between pt-3 border-t border-white/10 text-[11px]">
            ...
            <span className="text-white/80 hover:text-white cursor-pointer flex items-center gap-0.5">More info <Info className="h-3 w-3" /></span>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-[#2563eb] p-5 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex justify-between items-start">
            ...
            <div className="opacity-20"><FileText className="h-10 w-10" /></div>
          </div>
          <div className="flex items-center justify-between pt-3 border-t border-white/10 text-[11px]">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-[#f59e0b] p-5 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex justify-between items-start">
            ...
            <div className="opacity-20"><Users className="h-10 w-10" /></div>
          </div>
          <div className="flex items-center justify-between pt-3 border-t border-white/10 text-[11px]">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-[#7c3aed] p-5 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex justify-between items-start">
            ...
            <div className="opacity-20"><AlertTriangle className="h-10 w-10" /></div>
          </div>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-[#dc2626] p-5 text-white shadow-sm flex flex-col justify-between min-h-[140px]">
          <div className="flex justify-between items-start">
            ...
            <div className="opacity-20"><span className="text-3xl">💰</span></div>
          </div>
          <div className="flex items-center justify-end pt-3 border-t border-white/10 text-[11px]">
            <span className="text-white/80 hover:text-white cursor-pointer flex items-center gap-0.5">More info <Info className="h-3 w-3" /></span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 rounded-2xl bg-white p-4 shadow-sm border border-gray-100">
        <div className="flex items-center gap-3 p-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Package className="h-5 w-5" /></div>
          <div><p className="text-[11px] font-medium text-gray-500">Stock Value</p><h4 className="text-sm font-bold text-gray-900">{formatPrice(stats.stockValue)}</h4></div>
        </div>
        <div className="flex items-center gap-3 p-2">
          ...
          <div><p className="text-[11px] font-medium text-gray-500">Month Revenue</p><h4 className="text-sm font-bold text-gray-900">{formatPrice(stats.revenue)}</h4></div>
        </div>
        <div className="flex items-center gap-3 p-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-50 text-red-500"><TrendingDown className="h-5 w-5" /></div>
          <div><p className="text-[11px] font-medium text-gray-500">Month Expenses</p><h4 className="text-sm font-bold text-gray-900">₹0.00</h4></div>
        </div>
        <div className="flex items-center gap-3 p-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Scale className="h-5 w-5" /></div>
          <div><p className="text-[11px] font-medium text-gray-500">Month Net</p><h4 className="text-sm font-bold text-gray-900">{formatPrice(stats.revenue)}</h4></div>
        </div>
        <div className="flex items-center gap-3 p-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-purple-50 text-purple-600"><ShoppingCart className="h-5 w-5" /></div>
          <div><p className="text-[11px] font-medium text-gray-500">Average Order Value</p><h4 className="text-sm font-bold text-gray-900">{formatPrice(stats.avgOrder)}</h4></div>
        </div>
        <div className="flex items-center gap-3 p-2">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600"><Percent className="h-5 w-5" /></div>
          <div>
            <p className="text-[11px] font-medium text-gray-500">Conversion Rate</p>
            <div className="flex items-center gap-1.5"><h4 className="text-sm font-bold text-gray-900">2.45%</h4><span className="text-[10px] font-semibold text-emerald-600">↑ 12.5%</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-50">
              <span>Last 30 Days</span><ChevronDown className="h-3 w-3 text-gray-400" />
            </button>
          </div>
          <div className="h-[220px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.chartDays} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip formatter={(val: number) => [`₹${val}`, ""]} contentStyle={{ borderRadius: "10px", border: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.08)", fontSize: "12px" }} />
          ...
          <div className="flex items-center justify-center gap-6 mt-3 text-xs">
            <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#2563eb]" /><span className="text-gray-600 font-medium">Online Sales</span></div>
            <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#f97316]" /><span className="text-gray-600 font-medium">POS Sales</span></div>
          </div>
        </div>
          ...
          <div className="h-[220px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.chartDays} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip formatter={(val: number) => [`${val} visitors`, ""]} contentStyle={{ borderRadius: "10px", border: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.08)", fontSize: "12px" }} />
                <Line type="monotone" dataKey="visitors" name="Unique Visitors" stroke="#10b981" strokeWidth={2.5} dot={{ r: 3, fill: "#10b981" }} activeDot={{ r: 5 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="flex items-center justify-center gap-6 mt-3 text-xs">
            <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#10b981]" /><span className="text-gray-600 font-medium">Unique Visitors</span></div>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-50">
              <span>This Month</span><ChevronDown className="h-3 w-3 text-gray-400" />
            </button>
          </div>
          <div className="relative h-[180px] w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={paymentBreakdown} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={2} dataKey="value">
                  {paymentBreakdown.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} stroke="none" />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-base font-extrabold text-gray-900">{formatPrice(stats.revenue)}</span>
          ...
          <div className="flex items-center justify-center gap-6 mt-2 text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700 font-medium">Cash <span className="text-gray-400 text-[11px]">{stats.revenue > 0 ? `${formatPrice(stats.cashSales)} (${((stats.cashSales/stats.revenue)*100).toFixed(1)}%)` : "₹0.00 (0%)"}</span></span>
            </div>
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-sm bg-[#2563eb]" />
              <span className="text-gray-700 font-medium">Card <span className="text-gray-400 text-[11px]">{stats.revenue > 0 ? `${formatPrice(stats.onlineSales)} (${((stats.onlineSales/stats.revenue)*100).toFixed(1)}%)` : "₹0.00 (0%)"}</span></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => onNavigate?.("orders")} className="flex items-center gap-1 text-xs font-semibold text-gray-500 hover:text-gray-800">
              <span>View All</span><ChevronDown className="h-3 w-3" />
            </button>
          </div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-gray-50">
                {recentOrders.length === 0 ? (
                  <tr><td colSpan={6} className="py-8 text-center text-xs text-gray-400">No orders yet.</td></tr>
                ) : recentOrders.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="py-2.5">
                      <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold ${o.status === "Completed" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>{o.status}</span>
                    </td>
                    <td className="py-2.5">
                      <span className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-semibold ${o.source === "Online" ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-600"}`}>{o.source}</span>
                    </td>
                    <td className="py-2.5 text-right text-gray-400 hover:text-gray-600 cursor-pointer"><MoreVertical className="h-3.5 w-3.5 inline" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => onNavigate?.("products")} className="flex items-center gap-1 text-xs font-semibold text-gray-500 hover:text-gray-800">
              <span>View All</span><ChevronDown className="h-3 w-3" />
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-gray-50">
                {topSelling.length === 0 ? (
                  <tr><td colSpan={3} className="py-8 text-center text-xs text-gray-400">No products yet.</td></tr>
                ) : topSelling.map((p) => (
                  <tr key={p.name} className="hover:bg-gray-50/80 transition-colors">
                    <td className="py-2.5">
                      <div className="flex items-center gap-2">
                        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-100 text-sm">🏷️</span>
              ...
              return (
                <div key={i} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2.5">
                    <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${act.color}`}><Icon className="h-3.5 w-3.5" /></div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {[
          { label: "Total Customers", value: "128", change: "↑ 8.5%", changeColor: "text-emerald-600", data: sparklineData.customers, stroke: "#3b82f6" },
          { label: "Returning Customers", value: "24", change: "↑ 20%", changeColor: "text-emerald-600", data: sparklineData.returning, stroke: "#06b6d4" },
          { label: "Refunds", value: "₹0.00", change: "↓ 0%", changeColor: "text-red-500", data: sparklineData.refunds, stroke: "#ef4444" },
          { label: "Abandoned Carts", value: "7", change: "↓ 5%", changeColor: "text-red-500", data: sparklineData.abandoned, stroke: "#f97316" },
          { label: "Total Products", value: String(products.length), change: "↑ 6.2%", changeColor: "text-emerald-600", data: sparklineData.products, stroke: "#8b5cf6" },
          { label: "Reviews", value: "32", change: "↑ 14.5%", changeColor: "text-emerald-600", data: sparklineData.reviews, stroke: "#10b981" },
        ].map((card) => (
          <div key={card.label} className="rounded-2xl bg-white p-4 shadow-sm border border-gray-100 flex flex-col justify-between">
            <div>
