...
function SingleStickerPreview({
  ...
  if (cfg.rotateContent90) {
    return (
      <div
        className="relative flex items-center justify-center rounded-2xl border border-border bg-white text-black shadow-md overflow-hidden select-none shrink-0"
        style={{
          width: previewW,
          height: previewH,
        }}
      >
        ...
        >
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 whitespace-nowrap">
            Zérah Baby &amp; Kids
          </p>
          {showProductName && (
            <p className="font-semibold text-center line-clamp-1 px-1 text-[11px] whitespace-nowrap">
          ...
          {(showSellPrice || showMrp || showDiscount) && (
            <div className="flex items-center justify-center gap-1.5 whitespace-nowrap">
              {showSellPrice && (
                <span className="font-black text-xs">{priceFormatted}</span>
              )}
          ...
          <div className="w-full flex flex-col items-center">
            <Barcode
              ...
              lineColor="#000000"
            />
            <p className="mt-0.5 text-[8.5px] text-muted-foreground whitespace-nowrap">
              SKU: {product.sku || artNoVal}
            </p>
          </div>
        </div>
