...
export function generateBarcodeSvgString(
  ...
  try {
    ...
    // Grab the pixel dimensions generated by JsBarcode
    const originalWidth = svg.getAttribute("width") || "100";
    const originalHeight = svg.getAttribute("height") || "40";
    
    // Apply a viewBox so the browser scales the barcode vector graphics 
    // cleanly to whatever physical mm width/height we set.
    svg.setAttribute("viewBox", `0 0 ${originalWidth} ${originalHeight}`);
    // Omit preserveAspectRatio to use default "xMidYMid meet" (uniform scaling)
