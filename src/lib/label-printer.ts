...
export const PRINT_FORMAT_CONFIG: Record<string, PrintFormatConfig> = {
  /** 3 × 2 inch (76.2 × 50.8 mm) — Standard Landscape Thermal Barcode Tag */
  "3x2": {
    id: "3x2",
    name: "3 × 2 inch (76 × 50 mm)",
    shortLabel: "3×2 inch",
    pageWidthMm: 76.2,
    pageHeightMm: 50.8,
    pageMarginMm: 0,
    ...
    paddingHorizMm: 2.5,
    paddingBottomMm: 1.5,
    barcodeBarWidthPx: 1.35,
    barcodeHeightMm: 13.5,
    barcodeFontPt: 7.5,
    brandFontPt: 8.5,
    nameFontPt: 9.5,
    priceFontPt: 12.5,
...
export function generateBarcodeSvgString(
  ...
  try {
    ...
    const origWidth = parseFloat(svg.getAttribute("width") || "140");
    const origHeight = parseFloat(svg.getAttribute("height") || "50");

    svg.setAttribute("viewBox", `0 0 ${origWidth} ${origHeight}`);
    svg.setAttribute("preserveAspectRatio", "xMidYMid meet");

    // Compute proportional target physical mm size
    const targetHeightMm = cfg.heightMm + (cfg.displayValue !== false ? cfg.fontPt * 0.35 + 1.2 : 0);
    const aspect = origWidth / Math.max(1, origHeight);
    let targetWidthMm = targetHeightMm * aspect;
    if (targetWidthMm > cfg.maxWidthMm) {
      targetWidthMm = cfg.maxWidthMm;
    }
    const finalHeightMm = targetWidthMm / aspect;

    svg.setAttribute("width", `${targetWidthMm.toFixed(2)}mm`);
    svg.setAttribute("height", `${finalHeightMm.toFixed(2)}mm`);
    svg.setAttribute(
      "style",
      "display:block;margin:0 auto;overflow:visible;max-width:100%;text-align:center;",
    );
