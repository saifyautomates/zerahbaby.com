...
const ARTIFACT_DIR =
  "C:/Users/jackx/.gemini/antigravity-ide/brain/d0284833-e478-435a-abc2-783b981a1521";
...
test.describe("Visual Proof Capture Session", () => {
  ...
  test("Capture Visual Proof of Critical User & Admin Journeys", async ({ page }) => {
    ...
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "storefront_home.png"),
      fullPage: false,
    });
    ...
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "storefront_shop.png"),
      fullPage: false,
    });
    ...
    if ((await firstProduct.count()) > 0) {
      ...
      await page.screenshot({
        path: path.join(ARTIFACT_DIR, "storefront_pdp.png"),
        fullPage: false,
      });
    ...
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "storefront_cart.png"),
      fullPage: false,
    });
    ...
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "admin_dashboard.png"),
      fullPage: false,
    });
    ...
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "admin_pos_terminal.png"),
      fullPage: false,
    });
    ...
    await page.waitForTimeout(1500);
    await page.screenshot({
      path: path.join(ARTIFACT_DIR, "admin_online_orders.png"),
      fullPage: false,
    });
