-- Migration: 20260928000068_fix_store_credit_ledger_types_and_return.sql
-- Harmonize store_credit_ledger types and ensure process_offline_return passes validation

-- 1. Relax/extend store_credit_ledger_type_check constraint
...
ALTER TABLE public.store_credit_ledger
  ...
  CHECK (type = ANY (ARRAY[
    'CREDIT_ISSUED'::text,
    'CREDIT_USED'::text,
    'CREDIT_REDEEMED'::text,
    'issuance'::text,
    'redemption'::text,
    'adjustment'::text,
    'credit_added'::text,
    'credit_used'::text,
    'credit_refunded'::text,
    'CREDIT_EXPIRED'::text
  ]));

-- 2. Update process_offline_return to use standard 'CREDIT_ISSUED'
