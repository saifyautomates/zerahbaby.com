-- ==============================================================================
-- Migration: 20260928000010_drop_overloaded_process_offline_return.sql
-- Description:
-- Drop ambiguous old overloaded signatures of process_offline_return
-- so PostgREST resolves the single canonical 11-argument function cleanly.
-- ==============================================================================

-- 1. Drop old 10-parameter version
DROP FUNCTION IF EXISTS public.process_offline_return(text, text, text, uuid, text, text, text, text, jsonb, text);

-- 2. Drop any legacy versions with other signatures if present
DROP FUNCTION IF EXISTS public.process_offline_return(text, text, text, uuid, text, text, text, text, jsonb);

-- 3. Canonical single function with default parameters
