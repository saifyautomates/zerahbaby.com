-- =====================================================================
-- Migration: 20260928000163_fix_store_credit_ledger_source_sale_id.sql
-- Description: Ensure store_credit_ledger has source_sale_id, used_in_sale_id,
--              and sale_id columns so store credit redemptions during POS checkout
--              succeed without column missing errors, and update place_offline_sale.
-- =====================================================================

-- 1. Add all sale and return reference columns to store_credit_ledger
ALTER TABLE public.store_credit_ledger
  ADD COLUMN IF NOT EXISTS source_sale_id uuid REFERENCES public.offline_sales(id) ON DELETE SET NULL;

ALTER TABLE public.store_credit_ledger
  ADD COLUMN IF NOT EXISTS used_in_sale_id uuid REFERENCES public.offline_sales(id) ON DELETE SET NULL;

ALTER TABLE public.store_credit_ledger
  ADD COLUMN IF NOT EXISTS sale_id uuid REFERENCES public.offline_sales(id) ON DELETE SET NULL;

ALTER TABLE public.store_credit_ledger
  ADD COLUMN IF NOT EXISTS source_return_id uuid REFERENCES public.offline_returns(id) ON DELETE SET NULL;

ALTER TABLE public.store_credit_ledger
  ADD COLUMN IF NOT EXISTS return_id uuid REFERENCES public.offline_returns(id) ON DELETE SET NULL;

-- 2. Backfill null cross-reference columns if one is present
UPDATE public.store_credit_ledger
SET source_sale_id = COALESCE(source_sale_id, used_in_sale_id, sale_id),
    used_in_sale_id = COALESCE(used_in_sale_id, source_sale_id, sale_id),
    sale_id = COALESCE(sale_id, source_sale_id, used_in_sale_id)
WHERE source_sale_id IS NULL OR used_in_sale_id IS NULL OR sale_id IS NULL;

-- 3. Relax amount check constraints so positive and negative amounts never fail
ALTER TABLE public.store_credit_ledger DROP CONSTRAINT IF EXISTS store_credit_ledger_amount_check;
ALTER TABLE public.store_credit_ledger DROP CONSTRAINT IF EXISTS pos_store_credit_ledger_amount_check;
...
SELECT 
  ...
  balance_after,
  credit_token,
  COALESCE(source_return_id, return_id) as return_id,
  COALESCE(source_return_id, return_id) as source_return_id,
  COALESCE(used_in_sale_id, sale_id, source_sale_id) as sale_id,
  COALESCE(source_sale_id, used_in_sale_id, sale_id) as source_sale_id,
  COALESCE(used_in_sale_id, sale_id, source_sale_id) as used_in_sale_id,
  notes,
  created_by,
  created_at
...
-- 6. Canonical place_offline_sale RPC with full store_credit_ledger compatibility
