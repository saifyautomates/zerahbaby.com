-- Migration: 20260928000070_four_char_exchange_voucher_token.sql
-- Implement 4-character uppercase alphanumeric store credit voucher token format with cryptographic uniqueness

-- 1. Cryptographically strong 4-character token generator with active collision guard
...
BEGIN
  LOOP
    ...
    res := '';
    FOR i IN 1..4 LOOP
      res := res || substr(chars, (floor(random() * 36) + 1)::int, 1);
    END LOOP;
...
GRANT EXECUTE ON FUNCTION public.generate_unique_exchange_credit_token() TO authenticated, anon, service_role;

-- 2. Unique index on active credit tokens
CREATE UNIQUE INDEX IF NOT EXISTS idx_offline_returns_active_credit_token 
  ON public.offline_returns (UPPER(credit_token)) 
  WHERE (credit_token_status = 'ACTIVE' AND credit_token IS NOT NULL);

-- 3. Update canonical process_offline_return to use 4-character token generator
