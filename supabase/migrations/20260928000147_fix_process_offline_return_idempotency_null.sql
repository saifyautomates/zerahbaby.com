-- Migration: 20260928000147_fix_process_offline_return_idempotency_null.sql
-- Fix: POS Return failed with "null value in column refund_amount of relation offline_returns violates not-null constraint"
-- Root Cause: In process_offline_return, the idempotency check selected into scalar variables:
...
--   FROM public.offline_returns WHERE idempotency_key = ...
-- When no matching row was found (every new return), PL/pgSQL wiped all target variables to NULL,
-- including computed_total_refund and _customer_name.
-- Fix: Select into a single record variable (v_existing_return) so scalars are never wiped.
...
BEGIN
  ...
  -- 2. Idempotency Check (Select into a record so scalars are NOT wiped to NULL on non-match!)
