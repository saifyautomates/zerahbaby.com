-- ==============================================================================
-- Migration: 20260928000112_fix_place_order_order_items_price_at_time_column.sql
-- Description:
-- 1. Fix `place_order` RPC which was erroneously trying to insert into the non-existent
--    `price_at_time` column in the `order_items` table.
-- ==============================================================================
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id uuid, product_slug text, qty int) LOOP
    ...
    -- FIXED: Removed duplicate `quantity` and non-existent `price_at_time` from INSERT INTO
