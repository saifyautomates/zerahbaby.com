-- =====================================================================
-- Migration: 20260927000005_fix_place_order_payment_status_enum.sql
-- Description:
-- 1. Ensure public.payment_status enum contains all required states
-- 2. Update place_order to cleanly cast payment_status to 'pending'::public.payment_status
-- =====================================================================
...
-- Update place_order RPC to ensure reliable placement
...
BEGIN
  ...
  -- Validate and compute subtotal from actual DB variant prices
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id uuid, qty int) LOOP
    SELECT v.id AS variant_id, COALESCE(v.price_override, p.price) AS price, v.sku AS variant_sku,
           ...
           p.name AS product_name, p.id AS p_id, p.stock AS p_stock
    INTO variant
    FROM public.product_variants v
    JOIN public.products p ON p.id = v.product_id
    WHERE v.id = item.variant_id;
    ...
    INSERT INTO public.inventory_transactions (
      ...
      variant.p_id, variant.variant_id, 'sale'::public.inventory_tx_type, -item.qty, variant.p_stock, GREATEST(0, variant.p_stock - item.qty),
