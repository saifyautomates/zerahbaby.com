-- Migration: 20260928000288_fix_place_offline_sale_idempotency_subtotal_null.sql
-- Description: Fix null subtotal constraint violation in place_offline_sale caused by SELECT INTO wiping scalar variables on new sales
...
BEGIN
  -- 1. Idempotency Guard (Use separate record variable so calculation variables are NOT set to NULL when no row is found)
  ...
  -- 3. Calculate Subtotal from Items (defensively initialized and accumulated)
  v_subtotal := 0;
  ...
  -- 12. Insert offline_sale (guaranteed non-null numeric fields)
  ...
  FOR elem IN SELECT * FROM jsonb_array_elements(_items) LOOP
    -- Safe regex check for valid UUID before casting
