-- Harden inventory restock and variant stock mutation entry points.
-- Direct customer access to restore_stock_for_order is removed. Internal calls
-- from SECURITY DEFINER cancellation/trigger functions remain supported.
-- admin_update_variant_stock also rejects negative stock values server-side.

REVOKE EXECUTE
ON FUNCTION public.restore_stock_for_order(uuid, text, text)
