-- Migration: 20260928000157_fix_pos_customers_visits_count_columns.sql
-- Description: Add missing visits_count, last_visit columns to pos_customers and ensure place_offline_sale updates both canonical and alias fields safely.

-- 1. Ensure all customer columns exist on pos_customers
ALTER TABLE public.pos_customers
  ...
  ADD COLUMN IF NOT EXISTS last_visit_date timestamptz DEFAULT now(),
  ADD COLUMN IF NOT EXISTS store_credit numeric DEFAULT 0,
  ADD COLUMN IF NOT EXISTS store_credit_balance numeric DEFAULT 0;

-- 2. Sync values across aliases
UPDATE public.pos_customers
SET
  visits_count = COALESCE(NULLIF(visits_count, 0), NULLIF(total_visits, 0), 0),
  total_visits = COALESCE(NULLIF(total_visits, 0), NULLIF(visits_count, 0), 0),
  total_spent = COALESCE(NULLIF(total_spent, 0), NULLIF(total_spend, 0), 0),
  total_spend = COALESCE(NULLIF(total_spend, 0), NULLIF(total_spent, 0), 0),
  last_visit = COALESCE(last_visit, last_visit_date, now()),
  last_visit_date = COALESCE(last_visit_date, last_visit, now()),
  store_credit = COALESCE(store_credit, store_credit_balance, 0),
  store_credit_balance = COALESCE(store_credit_balance, store_credit, 0);

-- 3. Replace place_offline_sale RPC with robust customer and item tracking
...
BEGIN
  ...
  IF v_cust_id IS NULL AND v_clean_phone != '' AND length(v_clean_phone) >= 10 THEN
    ...
    IF v_cust_id IS NULL THEN
      INSERT INTO public.pos_customers (
        ...
        visits_count, total_visits, total_purchases,
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(_items) AS x(
    ...
    -- Insert into offline_sale_items populating all schema variations (name, product_name, qty, quantity)
