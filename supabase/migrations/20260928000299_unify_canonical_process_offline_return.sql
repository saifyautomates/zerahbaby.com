-- Migration: 20260928000299_unify_canonical_process_offline_return.sql
-- Description: Drop all stale overloaded signatures of process_offline_return and unify into ONE canonical SECURITY DEFINER function with universal parameter support, robust role authorization, atomic inventory restocking, and store credit synchronization.

-- 1. Drop all prior conflicting overloaded function signatures
...
DROP FUNCTION IF EXISTS public.process_offline_return(text, text, text, text, text, text, text, text, jsonb, text, uuid, text);
DROP FUNCTION IF EXISTS public.process_offline_return(text, text, text, uuid, text, text, text, text, jsonb, text, uuid, text);
...
DROP FUNCTION IF EXISTS public.process_offline_return(text, text, text, uuid, text, text, text, text, uuid, jsonb);

-- 2. Create the unified, canonical process_offline_return function
...
BEGIN
  -- 1. Authorization: Allow service_role and unauthenticated POS kiosk operations, verify staff/admin if uid is present
