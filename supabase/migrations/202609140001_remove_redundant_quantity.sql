-- ============================================================
-- FULL-STACK AUDIT: REMOVE REDUNDANT QUANTITY COLUMN
-- ============================================================

-- Drop the redundant quantity column from order_items
ALTER TABLE public.order_items DROP COLUMN IF EXISTS quantity;
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(product_slug text, qty int) LOOP
    ...
    -- [RESTORED] STRICT BACKEND CHECK FOR OFFLINE ONLY PRODUCTS
