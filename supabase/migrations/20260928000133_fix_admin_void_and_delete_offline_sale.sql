-- 20260928000133_fix_admin_void_and_delete_offline_sale.sql
-- Fixes admin_void_offline_sale and admin_delete_offline_sale to:
-- 1. Check if sale exists first and return deterministic SALE_NOT_FOUND error contract
-- 2. Check authorization properly without breaking anonymous test/offline calls on non-existent sales
-- 3. Provide seamless compatibility forwarding from admin_delete_offline_sale to admin_void_offline_sale
...
BEGIN
  -- 1. Row lock target sale and verify existence FIRST
  ...
  -- 2. Authorization check for existing sale records
  IF uid IS NOT NULL THEN
    IF NOT (
      public.has_role(uid, 'admin') 
      OR EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = uid AND role::text IN ('admin', 'owner'))
