-- =============================================================================
-- Migration: 20260928000136_fix_variant_unassigned_in_create_checkout_session.sql
-- Description: Fix 'record variant is not assigned yet' in create_checkout_session
--              by properly pre-initializing the variant record variable with defined
--              column structure, exactly matching the fix applied in place_order.
-- =============================================================================
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id text, product_slug text, product_id text, qty int) LOOP
    ...
    SELECT NULL::uuid AS variant_id, NULL::numeric AS price, NULL::numeric AS mrp,
           ...
           NULL::text AS product_slug, NULL::text AS product_name, NULL::uuid AS p_id
    INTO variant;
