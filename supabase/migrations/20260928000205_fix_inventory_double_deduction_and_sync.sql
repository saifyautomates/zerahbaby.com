-- =============================================================================
-- Migration: 20260928000203_fix_inventory_double_deduction_and_sync.sql
-- Description:
-- 1. Fix double-deduction in place_order RPC:
--    - Use atomic row locks (FOR UPDATE) on product_variants and products.
--    - Update product_variants.stock as single authoritative source.
--    - Rely on trg_sync_variant_to_product_stock trigger to update parent products.stock
--      without the redundant manual UPDATE on products.stock that doubled deductions.
--    - Only manually update products.stock when an item has no variant.
-- 2. Harden process_offline_return and restore_stock_for_order:
--    - Guarantee atomic, lockstep variant and product restocking.
--    - Ensure strict single-source updates across both online and offline channels.
-- =============================================================================
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id text, product_slug text, product_id text, qty int) LOOP
    ...
    IF variant.variant_id IS NOT NULL THEN
      ...
      -- DO NOT manually update public.products here to prevent double-deduction!
