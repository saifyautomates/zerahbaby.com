-- ==============================================================================
-- Migration: Fix lookup_barcode RPC for SKU return & Case-Insensitive Barcodes
-- ==============================================================================
...
BEGIN
  ...
  is_uuid := clean_input ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';
  ...
  SELECT v.id AS variant_id, v.sku AS variant_sku, v.name AS variant_name, v.color AS variant_color,
         ...
         p.sku AS product_sku, p.barcode AS product_barcode, p.is_active, p.age_group, p.description
  INTO match_record
  FROM public.product_variants v
  JOIN public.products p ON p.id = v.product_id
  WHERE (v.barcode ILIKE clean_input OR v.sku ILIKE clean_input)
  LIMIT 1;

  -- 2. If not found, try finding by parent barcode, parent SKU, parent slug, or parent ID
  IF match_record.product_id IS NULL THEN
    ...
    WHERE (
      p.barcode ILIKE clean_input 
      OR p.sku ILIKE clean_input 
      OR p.slug ILIKE clean_input 
      OR (is_uuid AND p.id::text = clean_input)
    )
  ...
  IF NOT match_record.is_active THEN
    RETURN jsonb_build_object(
      ...
      'sku', COALESCE(match_record.variant_sku, match_record.product_sku, match_record.product_barcode),
