-- Migration: 20260928000249_fix_finalize_paid_order_paise_normalization.sql
-- Description: Restore paise-to-rupee normalization and harmonize parameter signatures in canonical finalize_paid_order.

DROP FUNCTION IF EXISTS public.finalize_paid_order(text, text, text, text, numeric);
...
BEGIN
  ...
  IF _verified_amount IS NOT NULL AND _verified_amount > 0 THEN
    -- If passed in paise (standard in Razorpay webhooks/callbacks e.g. 28000 paise for ₹280), normalize to rupees
