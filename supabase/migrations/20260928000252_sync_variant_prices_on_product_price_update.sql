-- Migration: 20260928000252_sync_variant_prices_on_product_price_update.sql
-- Description: Ensure admin_update_product_price synchronizes all default/unnamed variants with the product price, and clean duplicate default variants.
...
BEGIN
  ...
  -- Synchronize all default/unnamed variants or single variant so pricing propagates consistently
  ...
  WHERE product_id = _product_id
    AND (lower(COALESCE(name, 'default')) = 'default' OR name IS NULL);
