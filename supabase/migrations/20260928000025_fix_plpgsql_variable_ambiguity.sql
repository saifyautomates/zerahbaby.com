-- Migration: 20260928000025_fix_plpgsql_variable_ambiguity.sql
-- Description: Prefix PL/pgSQL local variables with v_ to eliminate column reference ambiguity.
...
BEGIN
  ...
  IF _credit_token IS NOT NULL AND trim(_credit_token) != '' THEN
    ...
    IF v_token_record.id IS NOT NULL THEN
      IF v_resolved_customer_id IS NULL AND v_token_record.customer_id IS NOT NULL THEN
        v_resolved_customer_id := v_token_record.customer_id;
      END IF;
    END IF;
  END IF;
  ...
  IF v_resolved_customer_id IS NOT NULL THEN
    ...
    IF v_customer_record.id IS NOT NULL THEN
      v_current_customer_credit := COALESCE(v_customer_record.store_credit_balance, 0);
    END IF;
  ELSIF v_token_record.id IS NOT NULL THEN
    v_current_customer_credit := COALESCE(v_token_record.balance_after, 0);
  END IF;
