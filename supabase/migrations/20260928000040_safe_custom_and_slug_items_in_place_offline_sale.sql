-- Migration: 20260928000040_safe_custom_and_slug_items_in_place_offline_sale.sql
-- Description: Gracefully handle custom products, slugs, and non-UUID variant IDs without crashing place_offline_sale RPC
...
BEGIN
  ...
  LOOP
    ...
    -- Determine if product_id is a valid UUID
    ...
    -- Determine if variant_id is a valid UUID
    IF v_item.variant_id IS NOT NULL AND v_item.variant_id != '' THEN
      v_is_var_uuid := (v_item.variant_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$');
    END IF;

    -- Look up product & lock for atomic inventory deduction safely
    IF v_is_uuid THEN
    ...
    -- If standard catalog product resolved, deduct inventory
    IF v_prod_id IS NOT NULL THEN
      ...
      -- If valid variant specified, deduct variant stock as well
    ...
    -- product_id references public.products(id) and is NULLABLE for custom items
