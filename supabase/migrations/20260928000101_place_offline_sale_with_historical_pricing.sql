-- ==============================================================================
-- Migration: 20260928000101_place_offline_sale_with_historical_pricing.sql
-- Description:
--   Rewrite place_offline_sale to calculate and persist immutable per-line
--   historical pricing snapshots at sale time.
--
-- NEW logic in item INSERT:
--   1. unit_mrp            = item.mrp (from frontend cart)
--   2. unit_selling_price  = item.price (the line price set in cart)
--   3. line_gross_amount   = unit_selling_price × qty
--   4. allocated_bill_discount  = proportional share of whole-bill discount
--   5. allocated_coupon_discount = proportional share of coupon discount
--   6. final_unit_paid_price = unit_selling_price
--                              - allocated_bill_discount
--                              - allocated_coupon_discount
--   7. quantity_sold       = qty
--   8. quantity_returned   = 0 (no returns at sale time)
--
-- Allocation formula:
--   weight = line_gross / total_subtotal
--   allocated_bill = ROUND(weight * v_bill_only_discount / qty, 4)
--   allocated_coupon = ROUND(weight * v_coupon_discount / qty, 4)
--
-- NOTE: existing payment, inventory, voucher, and coupon logic is UNCHANGED.
-- ==============================================================================
...
DECLARE
  ...
  v_bill_only_discount numeric := 0;   -- whole-bill discount (excl. coupon)
...
BEGIN
  ...
  LOOP
    ...
    -- Proportional allocation of bill-level discount to this line
    v_alloc_bill := CASE
      WHEN v_subtotal > 0 THEN ROUND((v_line_gross / v_subtotal) * v_bill_only_discount / v_item_qty, 4)
      ELSE 0
    END;

    -- Proportional allocation of coupon discount to this line
    v_alloc_coupon := CASE
      WHEN v_subtotal > 0 THEN ROUND((v_line_gross / v_subtotal) * v_coupon_discount / v_item_qty, 4)
      ELSE 0
    END;

    -- Final net price per unit (what the customer actually paid per piece)
    ...
    INSERT INTO public.offline_sale_items (
      ...
      v_item_price,
      v_line_gross,
      0,                    -- product_discount_amount (reserved for future item-level discounts)
      v_alloc_bill,
      v_alloc_coupon,
      v_final_unit_paid,
      v_item_qty,
      0,                    -- quantity_returned starts at 0
      now()
    );
