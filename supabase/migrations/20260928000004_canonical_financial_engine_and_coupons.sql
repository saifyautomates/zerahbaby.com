-- ==============================================================================
-- Migration: 20260928000004_canonical_financial_engine_and_coupons.sql
-- Description:
-- 1. Standardize validate_coupon function with exact bounds, caps, and rounding
-- 2. Standardize place_order RPC with net subtotal free delivery threshold evaluation
-- 3. Standardize place_offline_sale with exact discount capping and integer-safe total
-- ==============================================================================

-- 1. Canonical validate_coupon Function
...
BEGIN
  ...
  SELECT * INTO c FROM public.coupons WHERE upper(code) = clean_code AND active;
  IF NOT FOUND THEN 
  ...
  IF _order_total < c.minimum_order_value THEN 
    RETURN jsonb_build_object('valid', false, 'error', 'Minimum order value is ₹' || c.minimum_order_value); 
  END IF;
  ...
  IF c.discount_type = 'percentage' THEN
    ...
    IF c.maximum_discount > 0 AND discount > c.maximum_discount THEN 
      discount := c.maximum_discount; 
    END IF;
  ELSE
...
GRANT EXECUTE ON FUNCTION public.validate_coupon TO authenticated, service_role;


-- 2. Master place_order RPC with Reconciled Financial Formulas
