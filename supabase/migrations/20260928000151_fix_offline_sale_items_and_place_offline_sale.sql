-- Migration: 20260928000151_fix_offline_sale_items_and_place_offline_sale.sql
-- Description: Add alias columns (name & product_name, qty & quantity) to offline_sale_items
-- and update place_offline_sale RPC to populate both safely.

-- 1. Ensure all column variations exist on offline_sale_items
ALTER TABLE public.offline_sale_items
  ADD COLUMN IF NOT EXISTS name text DEFAULT 'POS Item',
  ADD COLUMN IF NOT EXISTS product_name text DEFAULT 'POS Item',
...
-- Backfill name & product_name, qty & quantity
UPDATE public.offline_sale_items
SET 
  name = COALESCE(name, product_name, 'POS Item'),
  product_name = COALESCE(product_name, name, 'POS Item'),
  qty = COALESCE(qty, quantity, 1),
  quantity = COALESCE(quantity, qty, 1)
WHERE name IS NULL OR product_name IS NULL OR qty IS NULL OR quantity IS NULL;

-- 2. Update place_offline_sale RPC
