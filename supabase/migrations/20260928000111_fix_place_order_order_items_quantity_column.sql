-- ==============================================================================
-- Migration: 20260928000111_fix_place_order_order_items_quantity_column.sql
-- Description:
-- 1. Fix `place_order` RPC which was erroneously trying to insert into the dropped
--    `quantity` column in the `order_items` table instead of just using `qty`.
-- ==============================================================================
...
BEGIN
  ...
  IF _coupon_code IS NOT NULL AND trim(_coupon_code) != '' THEN
    ...
    IF coupon_record.id IS NOT NULL THEN
      IF (coupon_record.valid_from IS NULL OR now() >= coupon_record.valid_from) AND
         ...
         (coupon_record.min_order_amount IS NULL OR computed_subtotal >= coupon_record.min_order_amount) THEN

        IF coupon_record.discount_type = 'percent' OR coupon_record.discount_type = 'percentage' THEN
          computed_discount := ROUND((computed_subtotal * coupon_record.discount_value) / 100, 2);
          IF coupon_record.max_discount_amount IS NOT NULL AND coupon_record.max_discount_amount > 0 THEN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id uuid, product_slug text, qty int) LOOP
    ...
    -- FIXED: Removed duplicate `quantity` from INSERT INTO
    INSERT INTO public.order_items (
      ...
      new_order_id, variant.p_id, variant.variant_id, variant.product_slug, item.qty, variant.price, variant.price, (variant.price * item.qty), variant.variant_sku,
