-- Migration: 20260928000271_fix_default_variant_pricing_sync_and_checkout_matching.sql
-- Description:
-- 1. Ensure create_checkout_session and place_order always resolve Default variants dynamically to parent product price (never stale price_override).
-- 2. Update admin_update_product_price to always clear overrides on default variants so they dynamically inherit parent pricing.
-- 3. Add products trigger trg_sync_product_variant_price_override to reset default variants on product price update.
-- 4. Clean up any remaining default variants in product_variants table.

-- 1. Data Cleanup: Clear stale overrides on default variants
...
-- Specifically ensure all variants of tshirt-short inherit parent price
UPDATE public.product_variants
SET price_override = NULL,
    mrp_override = NULL
WHERE product_id IN (SELECT id FROM public.products WHERE slug = 'tshirt-short');

-- 2. Update create_checkout_session RPC
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(_items) AS x(variant_id text, product_slug text, product_id text, qty int, quantity int) LOOP
    ...
    IF v_clean_var_id IS NOT NULL THEN
      SELECT pv.id AS variant_id,
             CASE 
               WHEN (pv.color IS NULL OR trim(pv.color) = '') 
                AND (pv.size IS NULL OR trim(pv.size) = '') 
                AND (pv.name IS NULL OR trim(pv.name) = 'Default') 
               THEN p.price
             ...
             CASE 
               WHEN (pv.color IS NULL OR trim(pv.color) = '') 
                ...
                AND (pv.name IS NULL OR trim(pv.name) = 'Default') 
               THEN p.mrp
               ELSE COALESCE(pv.mrp_override, p.mrp)
             END AS mrp,
...
-- 3. Update admin_update_product_price to always clear overrides on default variants
