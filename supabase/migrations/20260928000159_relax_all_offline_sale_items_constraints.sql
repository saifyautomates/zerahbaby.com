-- Migration: 20260928000159_relax_all_offline_sale_items_constraints.sql
-- Description: Drop NOT NULL on all non-essential columns of offline_sale_items and provide safe defaults. Update place_offline_sale with COALESCE for all item fields.

-- 1. Relax constraints on public.offline_sale_items so missing fields never break checkout
...
ALTER TABLE public.offline_sale_items
  ALTER COLUMN product_slug SET DEFAULT 'item';

ALTER TABLE public.offline_sale_items
  ALTER COLUMN product_name DROP NOT NULL;
ALTER TABLE public.offline_sale_items
  ALTER COLUMN product_name SET DEFAULT 'Item';
...
UPDATE public.offline_sale_items SET variant_info = '' WHERE variant_info IS NULL;
UPDATE public.offline_sale_items SET barcode = '' WHERE barcode IS NULL;
UPDATE public.offline_sale_items SET sku = '' WHERE sku IS NULL;
UPDATE public.offline_sale_items SET product_slug = 'item' WHERE product_slug IS NULL;
UPDATE public.offline_sale_items SET name = 'Item' WHERE name IS NULL;
UPDATE public.offline_sale_items SET product_name = 'Item' WHERE product_name IS NULL;

-- 2. Update place_offline_sale to COALESCE all item columns
