-- ============================================================================
-- Migration: 20260928000315_harden_admin_inventory_adjustment.sql
-- Purpose:
--   Harden manual admin inventory adjustment authorization without changing
--   inventory behavior, calculations, transaction logging, or UI flows.
--
-- Scope:
--   - Only the admin_adjust_inventory RPC authorization surface changes.
--   - Existing admin/staff authorization semantics are preserved.
--   - Anonymous execution is removed.
--   - No sales, returns, refunds, cancellation, POS, or storefront logic is changed.
-- ============================================================================
...
DECLARE
  ...
  v_first_var_id uuid;
  v_first_var_prev_stock integer := 0;
...
BEGIN
  ...
  LOOP
    ...
    IF v_first_var_id IS NULL THEN
      ...
      v_first_var_prev_stock := v_row.stock;
    END IF;
  END LOOP;
  ...
  IF _variant_id IS NOT NULL THEN
    ...
    SET stock = v_final_stock, updated_at = now()
    WHERE id = variant.id;
    ...
    INSERT INTO public.inventory_transactions (
      ...
      prod.id, variant.id, 'adjustment'::public.inventory_tx_type,
      'adjustment'::public.inventory_tx_type, v_delta, v_prev_stock,
      v_final_stock, 'admin_manual_adjustment', variant.id,
      v_adj_reason, v_adj_reason, uid
    );
  ELSE
    IF v_var_count = 0 THEN
      ...
      INSERT INTO public.inventory_transactions (
        ...
        prod.id, NULL, 'adjustment'::public.inventory_tx_type,
        'adjustment'::public.inventory_tx_type, v_delta, v_prev_stock,
        v_final_stock, 'admin_manual_adjustment', prod.id,
        v_adj_reason, v_adj_reason, uid
      );
    ELSIF v_var_count = 1 THEN
      ...
      INSERT INTO public.inventory_transactions (
        ...
        prod.id, v_first_var_id, 'adjustment'::public.inventory_tx_type,
        'adjustment'::public.inventory_tx_type, v_delta, v_prev_stock,
        v_final_stock, 'admin_manual_adjustment', prod.id,
        v_adj_reason, v_adj_reason, uid
      );
    ELSIF v_delta >= 0 THEN
      UPDATE public.product_variants
      SET stock = v_first_var_prev_stock + v_delta, updated_at = now()
      WHERE id = v_first_var_id;
      ...
      INSERT INTO public.inventory_transactions (
        ...
        prod.id, v_first_var_id, 'adjustment'::public.inventory_tx_type,
        'adjustment'::public.inventory_tx_type, v_delta,
        v_first_var_prev_stock, v_first_var_prev_stock + v_delta,
        'admin_manual_adjustment', prod.id, v_adj_reason, v_adj_reason, uid
      );
    ELSE
      ...
      LOOP
        ...
        IF v_take > 0 THEN
          UPDATE public.product_variants
          SET stock = v_row.stock - v_take, updated_at = now()
          WHERE id = v_row.id;
          ...
          INSERT INTO public.inventory_transactions (
            ...
            prod.id, v_row.id, 'adjustment'::public.inventory_tx_type,
            'adjustment'::public.inventory_tx_type, -v_take,
            v_row.stock, v_row.stock - v_take,
            'admin_manual_adjustment', prod.id, v_adj_reason,
            v_adj_reason, uid
          );
...
REVOKE EXECUTE ON FUNCTION public.admin_adjust_inventory(uuid, uuid, integer, integer, text) FROM anon;
