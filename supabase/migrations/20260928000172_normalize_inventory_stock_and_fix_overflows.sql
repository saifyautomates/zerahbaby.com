-- =============================================================================
-- Migration: 20260928000172_normalize_inventory_stock_and_fix_overflows.sql
-- Description:
-- 1. Alter public.inventory_transactions columns to BIGINT to prevent integer out of range errors
-- 2. Normalize extreme/corrupted stock values on products and product_variants to realistic inventory levels
-- 3. Update place_cod_order and finalize_paid_order with BIGINT stock variables and zero-quantity guards
-- =============================================================================

-- 1. Ensure inventory_transactions quantity columns are BIGINT
DO $$ 
BEGIN
  ALTER TABLE public.inventory_transactions 
    ALTER COLUMN quantity TYPE bigint,
    ALTER COLUMN previous_quantity TYPE bigint,
    ALTER COLUMN new_quantity TYPE bigint;
EXCEPTION WHEN OTHERS THEN 
  NULL;
END $$;

-- 2. Normalize any overflowed or stress-test stock levels down to healthy standard retail stock
UPDATE public.products
SET stock = 250, updated_at = now()
WHERE stock > 10000;
...
-- 3. Canonical Update for place_cod_order with BIGINT stock variables
...
-- 4. Canonical Update for finalize_paid_order with BIGINT stock variables
