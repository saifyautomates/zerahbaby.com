-- ==============================================================================
-- Migration: 20260928000207_fix_place_order_not_null_fields.sql
-- Description:
-- Fix place_order INSERT INTO public.orders to use COALESCE instead of NULLIF
-- for NOT NULL columns: alt_phone, address_line2, landmark, notes.
-- ==============================================================================
...
BEGIN
  ...
  INSERT INTO public.orders (
    ...
    new_order_id, new_order_number, new_invoice, uid, trim(_full_name), trim(_email), trim(_phone), COALESCE(trim(_alt_phone), ''),
    trim(_address), COALESCE(trim(_address_line2), ''), COALESCE(trim(_landmark), ''), trim(_city), trim(_state), trim(_pincode),
    computed_subtotal, computed_discount, shipping, computed_total, lower(trim(_payment_method)), v_initial_payment_status,
    'placed', COALESCE(trim(_notes), ''), _coupon_code, v_clean_idem
  );
