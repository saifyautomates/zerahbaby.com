-- =====================================================================
-- Migration: 20260928000239_drop_stale_overloaded_pos_functions.sql
-- Description: Drop all stale/conflicting overloads of place_offline_sale
--              and process_offline_return so PostgREST resolves the single
--              canonical signature without any PGRST203 candidate ambiguity.
-- =====================================================================

-- 1. Drop stale place_offline_sale overloads
...
-- 2. Drop stale process_offline_return overloads
DROP FUNCTION IF EXISTS public.process_offline_return(
  text, text, text, uuid, text, text, text, text, jsonb, text
);

DROP FUNCTION IF EXISTS public.process_offline_return(
  text, text, text, uuid, text, text, text, text, jsonb
);

DROP FUNCTION IF EXISTS public.process_offline_return(
  text, text, text, uuid, text, text, text, text, jsonb, text, uuid
);

-- 3. Canonical Single place_offline_sale definition
