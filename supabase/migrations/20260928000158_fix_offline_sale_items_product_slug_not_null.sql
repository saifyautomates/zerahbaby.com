-- Migration: 20260928000158_fix_offline_sale_items_product_slug_not_null.sql
-- Description: Drop NOT NULL constraint on product_slug in offline_sale_items, set default 'item', and ensure place_offline_sale inserts product_slug safely.

-- 1. Ensure product_slug column is nullable and has safe fallback default
...
SET product_slug = 'item'
WHERE product_slug IS NULL;

-- 2. Update place_offline_sale RPC to parse and insert product_slug
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(_items) AS x(
    ...
    -- Insert into offline_sale_items populating all schema variations (name, product_name, qty, quantity, product_slug)
