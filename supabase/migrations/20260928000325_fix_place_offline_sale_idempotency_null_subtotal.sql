-- ==============================================================================
-- Migration: 20260928000325_fix_place_offline_sale_idempotency_null_subtotal.sql
-- Description: Fix null subtotal constraint violation in place_offline_sale.
--              PostgreSQL SELECT INTO scalar variables sets all variables to NULL
--              when no row matches, which wiped v_subtotal to NULL on every new sale.
--              We use a dedicated record variable (v_existing_sale) and explicit
--              COALESCE guards so subtotal and total can never be NULL.
-- ==============================================================================
...
BEGIN
  -- 1. Idempotency Guard: use dedicated record variable so scalar calculation variables are NEVER wiped to NULL!
  ...
  -- 12. Insert offline_sale with strict NOT NULL safety on subtotal and total
