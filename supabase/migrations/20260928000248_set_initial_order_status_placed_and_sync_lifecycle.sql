-- ==============================================================================
-- Migration: 20260928000248_set_initial_order_status_placed_and_sync_lifecycle.sql
-- Description:
-- 1. Sets initial order status to 'placed' in both finalize_paid_order and place_cod_order.
-- 2. Updates recent unfulfilled online orders (including ORD-260915-68883) to 'placed'.
-- 3. Ensures order_status_history records 'placed' on initial order creation.
-- ==============================================================================

-- 1. CANONICAL finalize_paid_order: Initial status strictly 'placed'
...
-- 2. CANONICAL place_cod_order: Initial status strictly 'placed'
...
BEGIN
  ...
  -- 6. Insert Order Items & Deduct Stock Atomically (SINGLE-SOURCE MUTATION)
  FOR item_rec IN SELECT * FROM jsonb_to_recordset(session_rec.items) AS x(
    variant_id uuid,
    product_id uuid,
    product_slug text,
...
-- 3. Retroactively set unfulfilled recent orders (including ORD-260915-68883) to 'placed'
UPDATE public.orders
SET status = 'placed',
    updated_at = now()
WHERE order_number = 'ORD-260915-68883'
   OR (
     status = 'processing'
