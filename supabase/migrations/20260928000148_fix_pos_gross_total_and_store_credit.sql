-- Migration: 20260928000148_fix_pos_gross_total_and_store_credit.sql
-- Description: Ensure place_offline_sale records gross sale total (subtotal - discount)
-- in offline_sales.total, records store_credit_used accurately, and returns payment_method
-- and gross total in the return object so thermal receipts and billing registers
-- display mathematically sound totals and settlement breakdown.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_to_recordset(_items) AS x(
    ...
    IF v_item.variant_id IS NOT NULL THEN
      SELECT COALESCE(price, 0) INTO v_item_price FROM public.product_variants WHERE id = v_item.variant_id;
    ELSE
...
-- Clean up any historical sale records where total was recorded as 0 due to store credit
UPDATE public.offline_sales
SET total = GREATEST(0, subtotal - discount)
WHERE total = 0 AND subtotal > 0 AND store_credit_used > 0;
