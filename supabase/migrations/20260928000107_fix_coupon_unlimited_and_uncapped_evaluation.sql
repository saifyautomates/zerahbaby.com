-- ==============================================================================
-- Migration: 20260928000107_fix_coupon_unlimited_and_uncapped_evaluation.sql
-- Description:
-- Fixes critical coupon evaluation edge cases in canonical place_order and
-- place_offline_sale RPCs:
-- 1. When max_uses / usage_limit is 0 or NULL, it represents an UNLIMITED coupon.
--    Previously, `used_count < max_uses` evaluated 0 < 0 as FALSE and rejected valid coupons.
-- 2. When max_discount_amount / maximum_discount is 0 or NULL, it represents an UNCAPPED discount.
--    Previously, `LEAST(computed_discount, 0)` forced the customer discount to 0.
-- 3. Synchronously increments both `used_count` and `usage_count` columns.
-- ==============================================================================

-- 1. Update place_order RPC
...
BEGIN
  ...
  -- 4. Dynamic Coupon Evaluation with Unlimited & Uncapped Support
  IF _coupon_code IS NOT NULL AND trim(_coupon_code) != '' THEN
    ...
    WHERE UPPER(code) = UPPER(trim(_coupon_code))
      ...
      -- Treat NULL, 0, or negative as unlimited usage
      AND (COALESCE(max_uses, usage_limit, 0) <= 0 OR COALESCE(used_count, usage_count, 0) < COALESCE(max_uses, usage_limit));

    IF FOUND THEN
      -- Check minimum order threshold (0 or NULL means no minimum)
      IF (COALESCE(coupon_record.min_order_amount, coupon_record.minimum_order_value, 0) <= 0 
          OR computed_subtotal >= COALESCE(coupon_record.min_order_amount, coupon_record.minimum_order_value, 0)) THEN

        IF coupon_record.discount_type = 'percentage' OR coupon_record.discount_type = 'percent' THEN
        ...
        -- Cap discount ONLY if maximum_discount / max_discount_amount > 0 (0 means unlimited / no cap)
        ...
        -- Synchronously increment both usage counters
