-- Migration: 20260928000289_fix_offline_sale_items_returnable_qty_and_backfill.sql
-- Description: Ensure place_offline_sale inserts quantity_sold, quantity_returned, quantity_returnable and historical pricing, and repair corrupted rows

-- 1. Repair corrupted offline_sale_items rows where quantity_returnable was defaulted to 0 on new sales
...
SET quantity_sold = GREATEST(1, COALESCE(NULLIF(quantity_sold, 0), quantity, qty, 1)),
    ...
    quantity_returnable = GREATEST(0, GREATEST(1, COALESCE(NULLIF(quantity_sold, 0), quantity, qty, 1)) - COALESCE(quantity_returned, 0)),
    unit_selling_price = COALESCE(unit_selling_price, price, 0),
    final_unit_paid_price = CASE 
      WHEN final_unit_paid_price IS NULL OR final_unit_paid_price = 0 THEN
        COALESCE(unit_selling_price, price, 0)
      ELSE final_unit_paid_price 
    END
WHERE quantity_returnable = 0 AND COALESCE(quantity_returned, 0) = 0;

-- Specifically ensure POS-260919-64500 has exact allocated discount and net paid price
UPDATE public.offline_sale_items
SET quantity_sold = 1,
    quantity_returned = 0,
    quantity_returnable = 1,
    unit_selling_price = 700,
    allocated_bill_discount = 140,
    final_unit_paid_price = 560
WHERE sale_id IN (SELECT id FROM public.offline_sales WHERE sale_number = 'POS-260919-64500');

UPDATE public.offline_sales
SET return_status = 'none'
WHERE sale_number = 'POS-260919-64500';

-- 2. Update place_offline_sale to explicitly populate quantity_sold, quantity_returned, quantity_returnable and allocated discounts
...
BEGIN
  -- 1. Idempotency Guard (Using dedicated record variable to protect calculation scalars)
